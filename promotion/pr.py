...
def render_body(
    *,
    ...
    lines += [
        ...
        "not modified while the Pull Request is being prepared. Review and manually "
        f"squash-merge this Pull Request to publish the change to `{destination}`. "
        "The author may perform the merge when repository protection permits.",
    ]
    lines += [
        "",
        "## Promotion lifecycle",
        ...
        f"- Workflow changes at PR creation: `{'YES' if lifecycle_metadata.has_workflow_changes else 'NO'}`",
        f"- Deployment action at PR creation: `{lifecycle_metadata.deployment_action}`",
        "- Final deployment action: recalculated from the files present when this PR is merged",
